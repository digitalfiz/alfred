...
const MASTER_EMAIL = process.env.GOOGLE_CLOUD_EMAIL; // This should probably be a new env var
...
bot.on('online', () => {
  ...
  bot.sendMessage(MASTER_EMAIL, 'ALFred Online!');  
});
...
bot.on('message', (from, message) => {
  ...
  console.log(`${sender}(${from})  >> ${message}`);
